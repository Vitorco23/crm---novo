...
const icebergAsset = icebergAssetImport;
const dashboardAsset = dashboardAssetImport;
const vitorAsset = vitorAssetImport;
const caseEnergia = caseEnergiaImport;
const caseAcademia = caseAcademiaImport;
const caseAcai = caseAcaiImport;
export function LPSections() {
    ...
    return (<div className="flex flex-col">
      ...
      <section id="diagnostico" className="py-24 px-6 bg-[#0b0b0d]">
        ...
        ].map((item) => (<div key={item.id} className="bg-[#151517] border border-[#2d2d2d] p-8 rounded-xl flex flex-col gap-4">
      ...
      <section id="metodo" className="py-24 px-6 bg-[#0b0b0d] border-t border-[#2d2d2d]">
        ...
        <div className="max-w-[1000px] mx-auto relative group">
          <img src={icebergAsset.url} alt="Modelo Iceberg da Engenharia de Receita" className="w-full h-auto rounded-3xl border border-[#caa55a]/20 shadow-2xl shadow-[#caa55a]/5 transition-transform duration-500 group-hover:scale-[1.01]" loading="lazy"/>
        </div>
      </section>
      ...
      <section id="cases" className="py-24 px-6 bg-[#0b0b0d] border-t border-[#2d2d2d]">
        ...
        <div className="max-w-[1180px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {cases.map((item, idx) => (<div key={idx} className="bg-[#151517] border border-[#2d2d2d] rounded-2xl overflow-hidden hover:border-[#caa55a]/50 transition-colors group">
              <div className="aspect-video overflow-hidden">
                <img src={item.image} alt={item.cat} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy"/>
              </div>
              <div className="p-8 flex flex-col gap-4">
                ...
                <ul className="flex flex-col gap-2 mt-2">
                  {item.results.map((r, i) => (<li key={i} className="text-[#b8b8b8] text-sm flex items-start gap-2">
      ...
      <section className="py-24 px-6 bg-[#151517] border-y border-[#2d2d2d]">
        <div className="max-w-[1180px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        ].map((bullet) => (<li key={bullet} className="flex items-center gap-4 text-[#f5f5f5] font-medium">
          ...
          <div className="rounded-3xl border border-[#2d2d2d] shadow-3xl overflow-hidden relative group">
             <img src={dashboardAsset.url} alt="Interface CRM Performance21" className="w-full h-auto grayscale group-hover:grayscale-0 transition-all duration-700" loading="lazy"/>
          </div>
        </div>
      </section>
      ...
      <section id="sobre" className="py-24 px-6 bg-[#0b0b0d]">
        <div className="max-w-[1180px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-5 rounded-3xl border border-[#2d2d2d] overflow-hidden grayscale hover:grayscale-0 transition-all duration-500 shadow-2xl">
            <img src={vitorAsset.url} alt="Vítor Oliveira" className="w-full h-auto scale-105" loading="lazy"/>
          </div>
      ...
      <section id="faq" className="py-24 px-6 bg-[#0b0b0d] border-t border-[#2d2d2d]">
        ...
        ].map((faq, idx) => (<details key={idx} className="group bg-[#151517] border border-[#2d2d2d] rounded-2xl overflow-hidden">
      ...
      <section className="py-24 px-6 bg-[#0b0b0d] border-t border-[#2d2d2d] mb-20">
        <div className="max-w-[1180px] mx-auto bg-gradient-to-br from-[#151517] to-[#1c1c20] border border-[#2d2d2d] rounded-[2.5rem] p-12 md:p-24 text-center flex flex-col items-center gap-8 relative overflow-hidden">
           ...
           <button onClick={() => document.getElementById("hero")?.scrollIntoView({ behavior: "smooth" })} className="z-10 bg-[#caa55a] text-[#0b0b0d] px-10 py-5 rounded-full font-bold text-lg hover:scale-105 transition-transform shadow-xl shadow-[#caa55a]/20">
