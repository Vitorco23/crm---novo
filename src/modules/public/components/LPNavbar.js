...
const logoAsset = logoAssetImport;
export function LPNavbar() {
    ...
    return (<header className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${scrolled
      ...
      <div className="max-w-[1180px] mx-auto flex h-20 items-center justify-between px-6">
        <a href="#top" onClick={(e) => scrollToSection(e, "#top")} className="flex items-center gap-2">
          <img src={logoAsset.url} alt="Performance21 Logo" className="h-8 w-auto rounded-lg brightness-125"/>
        ...
        <nav className="flex items-center gap-4 sm:gap-8">
          {links.map((l) => (<a key={l.href} href={l.href} onClick={(e) => scrollToSection(e, l.href)} className="text-[10px] sm:text-xs uppercase tracking-[0.14em] text-[#b8b8b8] transition-colors hover:text-[#f5f5f5]">
              {l.label}
            </a>))}
        </nav>
